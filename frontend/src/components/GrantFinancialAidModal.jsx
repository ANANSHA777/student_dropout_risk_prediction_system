      ...
      <div className="bg-[#0f172a] border border-slate-800 rounded-xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            Allocate College Emergency Fund
          </h3>
        ...
        <p className="text-sm text-slate-300 mb-4">
          Allocate institutional financial relief for <strong className="text-white">{student.name}</strong> to prevent dropout. This marks student status as <span className="text-emerald-400 font-semibold">"Pending Institutional Support"</span>.
        </p>
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1">Support Category</label>
            <input
              type="text"
              required
              value={reason}
          ...
          <button
            type="submit"
            className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-lg text-sm transition cursor-pointer"
          >
            Submit Request & Mark Pending Support
          </button>
        </form>
      </div>
    </div>
